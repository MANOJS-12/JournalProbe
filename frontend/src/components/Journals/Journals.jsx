...
const Search = () =>{
    ...
    return(
        <div className='p-4'>
            ...
            />
            {console.log(search)}
            <button className='h-10 w-20 bg-cyan-500 m-4 rounded-lg'>Search</button>
        </div>
    )
}
    ...
    <>
        ...
        <div className='p-4'>
            <input 
                type='text' 
                placeholder='Search the Journal...'
                className='border-0 border-b-2 border-solid border-b-cyan-500 md:min-w-[10rem] w-[50%] h-10 sm:p-2 bg-[#f3f4f5]'
                onChange={(e) => setSearch(e.target.value)}
            />
            <button className='h-10 w-20 bg-cyan-500 m-4 rounded-lg' onClick={()=>handleSearch()}>Search</button>
        </div>
        </div>
        <div>
            {filteredData.map((journal, index) =>
             (
                <Link to={`/${journal._id}`} key={index} className={`flex ${(index%2 === 0)?'flex-column':'lg:flex-column-reverse'} sm:p-4 m-8 shadow-md text-[#333]`}>
